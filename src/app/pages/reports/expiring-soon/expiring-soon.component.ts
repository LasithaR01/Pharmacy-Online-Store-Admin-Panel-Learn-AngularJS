...
@Component({
  ...
  styleUrls: ["./expiring-soon.component.scss"],
})
export class ExpiringSoonComponent implements OnInit {
  ...
  loadLowStockProducts() {
    this.productService.getExpiring().subscribe({
      next: (data) => {
        this.products = data;
  ...
  downloadReport() {
    this.reportService.expiringSoonProducts().subscribe({
      next: (response: Blob) => {
        const blob = new Blob([response], { type: "application/vnd.ms-excel" });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement("a");
        a.href = url;
        a.download = "expiring-soon-products.xls";
